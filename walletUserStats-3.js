...
async function showBetProgress() {
  ...
  try {
      ...
      const totalBet = result.data.walletUserStats.totalBet || 0;
...
function displayProgressBars(totalBet) {
  ...
  addMilestones(progressStage1, milestones.stage1, 5200);
  addMilestones(progressStage2, milestones.stage2, 5000);
  addMilestones(progressStage3, milestones.stage3, 10000);
}

function addMilestones(progressElement, milestones, range) {
  const milestoneContainer = document.createElement("div");
  milestoneContainer.className = "milestone-container";
  progressElement.parentElement.appendChild(milestoneContainer);

  milestones.forEach((milestone) => {
    const percentage = (milestone / range) * 100;
    ...
    milestoneContainer.appendChild(line);
    milestoneContainer.appendChild(label);
  });
}
