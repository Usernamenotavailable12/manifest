  ...
  async function openBox(userBoxId) {
    ...
    return data.data.openUserBox.userBox.reward.action.map(action => {
      if (action.bonus) {
        return { type: "BONUS", name: action.bonus.name, description: action.bonus.description, contentId: action.bonus.contentId };
      } else if (action.box) {
  ...
  function displayRewards(rewards) {
    ...
    rewardTimeout = setTimeout(clearRewards, 4000000);
  }
